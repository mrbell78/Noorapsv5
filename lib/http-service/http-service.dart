...
class HttpService {
  //late Dio _dio;
  var _dio;
  var token = Pref.getString("userid");
  String? getUserId(){
    if(Pref.getString("userid")!=null){
      print("the auth token is : ${Pref.getString("userid")}");
  ...
  Future<ApiResponse<Response>> getRequest(String route, {Map<String, String>? qp}) async {
    ...
    try {
      ...
      if (e.type == DioErrorType.response) {
        ...
        return ApiResponse(httpCode: int.parse(e.response!.statusCode.toString()), message: "${e.response!.statusMessage}", data: e.response!);
      } else {
        print(e.message);
  ...
  Future<ApiResponse<Response>> postRequest(String route, {Map<String, dynamic>? data, String? jsonData, bool isFormData = false, Function(int sent, int total)? onProgress,}) async {
    ...
    try {
      ...
      print("request body is : ${jsondata}");
      ...
      if (e.type == DioErrorType.response) {
        ...
        print(e.message);
        return ApiResponse(httpCode: -1, message: "Connection error. ${e.message}", data: e.response!.data!=null?e.response!.data!:e.stackTrace);
      }
    }
  }
